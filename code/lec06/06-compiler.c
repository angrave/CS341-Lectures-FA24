...
#include <sys/wait.h>
	...
	int main(int argc, char** argv) {
	  if(argc != 2) {
	    fprintf(stderr,"Example use: %s prog.c\n",argv[0]);
	    exit(1); 
	  }
	  char* target = argv[1];
	  while(1) {
...
//2. Terminate list of arguments with (const char*)NULL
        ...
        execlp(compiler, compiler,"-g",target, (char*) NULL);
	    ...
	    waitpid(child, & status, 0); // this will block until child has exited
